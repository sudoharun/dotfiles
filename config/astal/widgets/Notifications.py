...
class Notification(Gtk.Box):
    ...
    def sync(self, _: Notifd.Notification, id: str, *args):
        ...
        notif_container.timeout_id = GLib.timeout_add(5000, dismiss)

        def on_destroy(container, *args):
            if notif_container.timeout_id is not None:
                GLib.source_remove(notif_container.timeout_id)

        notif_container.connect("destroy", on_destroy)
